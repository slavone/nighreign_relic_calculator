window.__LOCALE_JP__ = {
  "ui": {
    ...
    "meth_formula_p2": "プールが<em>m</em>グループ（m ≥ K）にまたがる場合、確率は次の通りです:",
    "meth_formula_p3": "合計はプールからK個のグループを選ぶC(m, K)通り全てにわたります。各K個グループ集合Sについて、<code>ef(S, W)</code>はグループ除外を考慮します — グループが引かれると、そのウェイトがプールから除かれます。各グループの寄与ウェイトは選択したエフェクトのウェイトの合計です。ef式はK!通りの引き順全体を平均します:",
    "meth_formula_p4": "グループ内の全エフェクトを選択すると、そのグループの寄与ウェイトが最大化されます。特定の遺物色が必要な場合は最終結果に0.25を乗算します。",
